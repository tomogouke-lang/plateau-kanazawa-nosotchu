(function(){
    ...
    window.__pdsAvailabilityCallback=response=>{
      try{
        const rows=(response.table?.rows||[]).map(row=>({
          ...
          .filter(row=>!config.openDays?.length||config.openDays.includes(String(row.day)));
  ...
  function initPrice(config){
    ...
    if(!service) return;
    const care=$("careLevel"),time=$("timeSlot"),burden=$("burden"),weekly=$("weekly"),transport=$("transport"),lunch=$("lunch"),snack=$("snack"),bath=$("bath"),training=$("training");
    const careField=$("careLevelField"),timeField=$("timeSlotField"),weeklyField=$("weeklyField"),bathField=$("bathField"),trainingField=$("trainingField");
    ...
    function calc(){
      ...
      const visits=isDaycare
        ?Math.round(Number(weekly.value)*4.3)
        :Number(config.preventiveVisits?.[service.value]??(service.value==="preventive1"?4:8));
      let insurance=0;
      ...
      if(isDaycare){
        ...
        if(training&&!training.checked&&slot.trainingIncluded) trainingAdjustment=-Number(config.trainingFee||0)*rate*visits;
        insurance+=trainingAdjustment;
      ...
      const snackFee=snack?.checked?Number(config.snackFee||0)*visits:0;
      const transportRate=isDaycare
        ?Number(config.transportDeduction||0)
        :Number(config.preventiveTransportDeduction??config.transportDeduction??0);
      const transportDeduction=Number(transport.value)*transportRate*rate*visits;
      const sum=Math.max(0,insurance+lunchFee+snackFee-transportDeduction);
    ...
    [service,care,time,burden,weekly,transport,lunch,snack,bath,training].forEach(element=>element?.addEventListener("change",calc));
    calc();
  }
